...
import org.doubango.imsdroid.Screens.ScreenChat;
...
public class Main extends ActivityGroup implements IBaseScreen{
    ...
    //	menu.clear();//Add by vijay
    	...
    	if(activeSessionsCount == 0){onPrepareOptionsMenu( menu);}*/
	try{	if(mScreenService.getCurrentScreen().hasMenu()){
		Log.e("main thread", "main class exception In main has menu option");
			return mScreenService.getCurrentScreen().createOptionsMenu(menu);
		}
	}
	catch(ClassCastException e){
		Log.e("main thread", "main class exception");
	}
	...
	public boolean onPrepareOptionsMenu(Menu menu){
		try{
			Log.e("vijaypratapsing","vijaypratapsingh");
			if(mScreenService.getCurrentScreen().hasMenu()){
				menu.clear();
				mScreenService.getCurrentScreen().createOptionsMenu(menu);
				return mScreenService.getCurrentScreen().createOptionsMenu(menu);
			}
			}
			catch(ClassCastException e){
				Log.e("Main Class","exception in main line 131");
			//	return mScreenService.getCurrentScreen().createOptionsMenu(menu);
				//return ScreenAbout.FOCUSED_STATE_SET.createOptionsMenu(menu);
	...
	//	return true;//vijay
	/*	//Log.e((IBaseScreen)((Main)Engine.getInstance().getMainActivity()).getLocalActivityManager().getCurrentActivity());
		//System.out.println((IBaseScreen)((Main)Engine.getInstance().getMainActivity()).getLocalActivityManager().getActivity()());
		final int activeSessionsCount = NgnAVSession.getSize(new NgnPredicate<NgnAVSession>() {
			@Override
			public boolean apply(NgnAVSession session) {
				return session != null && session.isActive();
		...
		//if(mScreenService.getCurrentScreen().hasMenu()){
			//menu.clear();
			//return ScreenAbout.populateMenu(menu);
			return true;//false;
			
			//return mScreenService.getCurrentScreen().createOptionsMenu(menu);
		}
		
		else return mScreenService.getCurrentScreen().createOptionsMenu(menu);
		//return false;
*/	}
	...
	/*	if(mScreenService == null){
		...
		if(screen != null){
			...
			Log.e("main class"," onsaceInstancestate line 159");
		}
		super.onSaveInstanceState(outState);
	}
   ...
   // @Override
    public boolean onKeyDown(int keyCode, KeyEvent event) {
    	...
    	try{if(!BaseScreen.processKeyDown(keyCode, event)){
    		return super.onKeyDown(keyCode, event);
    	}}
    	catch(Exception ClassCastException){
    		Log.e("main class"," onsaceInstancestate line line 238");
    		return super.onKeyDown(keyCode, event);//added by vijay
    	}
    ...
    private void handleAction(Bundle bundle){
		...
		switch(bundle.getInt("action", Main.ACTION_NONE)){
			...
			case ACTION_RESTORE_LAST_STATE:
				...
				final SCREEN_TYPE screenType = NgnStringUtils.isNullOrEmpty(screenTypeStr) ? SCREEN_TYPE.ABOUT_T:
				...
				switch(screenType){
					case AV_T:
						...
						break;
					case CHAT_T:	
						mScreenService.show(ScreenChat.class);
						break;
					case QOS_T:	
