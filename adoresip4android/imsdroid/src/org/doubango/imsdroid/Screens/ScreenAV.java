...
public class ScreenAV extends BaseScreen{
	...
	//vijay
	private ToggleButton  holdcall,speakerButton,sendviedo,hold1, speaker1,sendviedo1;
	...
	private OnClickListener hold_OnClickListener= new OnClickListener(){
	 	@Override
		public void onClick(View v) {
	...
	private OnClickListener sendviedo_OnClickListener= new OnClickListener(){
	 	@Override
		public void onClick(View v) {
	...
	public boolean createOptionsMenu(Menu menu){
		...
		MenuItem itemSendStopVideo = null;
	      ...
	      MenuItem itemPickUp = menu.add(0, ScreenAV.MENU_PICKUP, 0, getString(R.string.string_answer)).setIcon(R.drawable.phone_pick_up_48);
			...
			MenuItem itemShareContent = menu.add(1, ScreenAV.MENU_SHARE_CONTENT, 0, "Stop Viedo").setIcon(R.drawable.video_stop_48);
			...
			switch(mAVSession.getState()){
				...
				case INCALL:
				{
					...
					if(itemSendStopVideo != null){
						...
						itemShareContent.setTitle("Answer").setIcon(R.drawable.phone_pick_up_48);
						itemShareContent.setEnabled(false);
					}
					else{
						itemPickUp.setEnabled(false);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		switch(item.getItemId()){
			...
			{if(mIsVideoCall){
				//itemSendStopVideo.setTitle(getString(R.string.string_switch_camera));
				 startStopVideo(!mAVSession.isSendingVideo());
			}
				 break;
			}
	...
	private void loadTryingView(){
		...
		//mCurrentView(ViewTrying);
		if(mViewTrying == null){
			mViewTrying = mInflater.inflate(R.layout.avcallpr, null);
			mViewTrying.dispatchWindowFocusChanged(true);//();//bringToFront();// = mInflater.inflate(R.layout.avcallpr, null);
		...
		//mTvDuration = (TextView)mViewTrying.findViewById(R.id.view_textView_duration);
		final ToggleButton bthold= (ToggleButton)mViewTrying.findViewById(R.id.view_call_trying_imageButton_hang123);
		final ToggleButton btspeaker= (ToggleButton)mViewTrying.findViewById(R.id.speakerbutton);
		final ToggleButton btviedo= (ToggleButton)mViewTrying.findViewById(R.id.view_call_trying_imageButton_hang12);
		holdcall=(ToggleButton)this.findViewById(R.id.view_call_trying_imageButton_hang123);
		speakerButton=(ToggleButton)this.findViewById(R.id.speakerbutton);
		sendviedo=(ToggleButton)this.findViewById(R.id.view_call_trying_imageButton_hang12);
		/*this.speakerButton.setOnClickListener(speakerbutton_OnClickListener);
		this.holdcall.setOnClickListener(hold_OnClickListener);
        this.sendviedo.setOnClickListener(sendviedo_OnClickListener);*/
		...
		btviedo.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				startStopVideo(!mAVSession.isSendingVideo());
			}
	...
	private void loadInCallAudioView(){
		...
		final ToggleButton bthold1= (ToggleButton)mViewInAudioCall.findViewById(R.id.toggle1);
		final ToggleButton btspeaker1= (ToggleButton)mViewInAudioCall.findViewById(R.id.toggle2);
		final ToggleButton btviedo1= (ToggleButton)mViewInAudioCall.findViewById(R.id.toggle3);
		hold1=(ToggleButton)this.findViewById(R.id.toggle1);
		speaker1=(ToggleButton)this.findViewById(R.id.toggle2);
		sendviedo1=(ToggleButton)this.findViewById(R.id.toggle3);
		btHang.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				hangUpCall();
		...
		//Add by vijay
		bthold1.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				if(mAVSession.isLocalHeld()){
					mAVSession.resumeCall();
					Log.e(TAG, "vijay is doing resume");
				}
				else{
					mAVSession.holdCall();
					Log.e(TAG, "vijay is doing hold");
				}
			}
		});
		btspeaker1.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				Log.e(TAG, "vijay is doing on speaker");
				mAVSession.toggleSpeakerphone();
			}
		});
		btviedo1.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View v) {
				Log.e(TAG, "vijay is sending viedo");
				startStopVideo(!mAVSession.isSendingVideo());
			}
		});
	...
	private final TimerTask mTimerTaskInCall = new TimerTask(){
		...
		public void run() {
			if(mAVSession != null && mTvDuration != null){
				...
				final Date date = new Date(new Date().getTime() - mAVSession.getStartTime() - 1800000);
				ScreenAV.this.runOnUiThread(new Runnable() {
					public void run() {
						//mTvDuration.setText("00.00");//xt(sDurationTimerFormat.format(date)=="00.00");
